...
### 2026-08-05 — Filter aligns with drawing title bar (not project index)
- **Symptom:** Filter was wrongly moved to the project-index column; Jacky wants it on the drawing-title vertical line (right pills / red arrow).
- **Fix:** `.site-filter { right: calc(margin + drawing-marker + drawing-gap) }` + `align-items: flex-end` so "filter" shares the label text’s right edge (before ◀). Restore index `top: var(--site-top)`.
- **Prevent:** Drawing bar right edge ≠ label text right edge — always inset by marker+gap when aligning chrome to drawing titles.
